                  ...
                  if (isNaN(v)) {
                    ...
                    if (!attr.precision) {
                      if (!isValidScale(v, attr.scale)) {
                        var msg = createMessage(key, "scale", "error_scale", resource, attr.resource);
                        errors.push(createError(path, na, "scale", msg));
                      }
                    }
                    else {
                      if (!isValidPrecision(v, attr.precision, attr.scale)) {
                        var msg = createMessage(key, "precision", "error_precision", resource, attr.resource);
              ...
              case "object":
                if (Array.isArray(v)) {
                  switch (at) {
                    case "strings": {
                      if (!isStrings(v)) {
                        var msg = createMessage(key, "strings", "error_strings", resource, attr.resource);
                    ...
                    case "numbers": {
                      if (!isNumbers(v)) {
                        var msg = createMessage(key, "numbers", "error_numbers", resource, attr.resource);
                    ...
                    case "integers": {
                      if (!isIntegers(v)) {
                        var msg = createMessage(key, "integers", "error_integers", resource, attr.resource);
                    ...
                    case "datetimes": {
                      if (!isDates(v)) {
                        var msg = createMessage(key, "datetimes", "error_datetimes", resource, attr.resource);
                    ...
                    case "dates": {
                      if (resources.ignoreDate) {
                        if (!isDates(v)) {
                          var msg = createMessage(key, "dates", "error_dates", resource, attr.resource);
                                ...
                                if (v[i]) {
                                  ...
                                  if (date3) {
                                    ...
                                    if (!(date3 instanceof Date) || error3 === "Invalid Date") {
                                      var y = path != null && path.length > 0 ? path + "." + key + "[" + i + "]" : key + "[" + i + "]";
                                      var msg = createMessage(key, "date", "error_date", resource, attr.resource);
                                      var err = createError("", y, "date", msg);
                                      errors.push(err);
                                    }
                                  }
                                }
...
function check(obj, attributes, allowUndefined, patch, resource, max) {
  var errors = [];
  var path = "";
  if (max == null) {
    max = undefined;
...
exports.check = check;
function validate(obj, attributes, allowUndefined, patch, resource, max) {
...
var Validator = (function () {
  ...
  Validator.prototype.validate = function (obj, patch, resource) {
    var errors = check(obj, this.attributes, this.allowUndefined, patch, resource, this.max);
